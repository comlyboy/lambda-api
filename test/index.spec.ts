...
test('test GET ApiGateway event', async () => {

	const response = await bootstrap(mockAPIGatewayProxyEventV2, mockContext);
	expect(response.statusCode).toBe(200);
	// expect(JSON.parse(response.body)).toEqual({ message: 'Hello, Jest!' });
});